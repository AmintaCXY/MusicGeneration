...
function userSaidGo() {
  model = new mm.MusicRNN("http://0.0.0.0:8000/checkpoints" +
  "/music_rnn_basic_rnn");
  model.initialize();
...
function playOrPause() {
  ...
  if (player.isPlaying()) {
    ...
    const sequence = board.getNoteSequenceUQ();
...
function play() {
  ...
  const sequence = mm.sequences.mergeConsecutiveNotes(board.getNoteSequenceUQ());
...
function infill() {
  ...
  const sequence = previousSequence = mm.sequences.quantizeNoteSequence(board.getNoteSequence(),1);
  
  writeNoteSeqs('melody-cont-inputs', [sequence],true);
  ...
  model.continueSequence(sequence, 64-sequence.notes.length, parseFloat(inputTemp.value))
  .then(output => {
    clearError();
    const EndStep = sequence.notes[sequence.notes.length-1].quantizedEndStep;

    const notes = []

  for (let i = 0; i < sequence.notes.length; i++) {
      const pair = sequence.notes[i];
      
      notes.push({pitch: pair.pitch, instrument: pair.instrument|| 0, 
                  quantizedStartStep: pair.quantizedStartStep, quantizedEndStep: pair.quantizedEndStep});    
    }

  for (let i = 0; i < output.notes.length; i++) {
      const pair = output.notes[i];
      
      notes.push({pitch: pair.pitch, instrument: pair.instrument|| 0, 
                  quantizedStartStep: pair.quantizedStartStep+EndStep, quantizedEndStep: pair.quantizedEndStep+EndStep});    
    }
  const ns = {};
  ns.notes = notes;

    writeNoteSeqs('melody-cont-results', [ns]);
    ...
    // Style the continue notes differently.
    for (let i = 0; i < ns.notes.length; i++) {
      ...
      {
        if(note.quantizedEndStep > note.quantizedStartStep+1 && note.quantizedEndStep<=PIXELS_WIDTH)
        for(let i = note.quantizedStartStep; i < note.quantizedEndStep; i++)
        {
          const uiButton = document.querySelector(`.pixel[data-row="${MAX_PITCH - note.pitch}"][data-col="${i}"]`);
          uiButton.classList.add('infilled');

        }
        else{
          const uiButton = document.querySelector(`.pixel[data-row="${MAX_PITCH - note.pitch}"][data-col="${note.quantizedStartStep}"]`);
          uiButton.classList.add('infilled');
...
function save() {
  const seq = mm.sequences.mergeConsecutiveNotes(board.getNoteSequenceUQ());
  var date=new Date();
   ...
   let str = 'JazzMIDI-'+year+mon+day+h+m+s+'.mid'
  saveAs(new File([mm.sequenceProtoToMidi(seq)], str));
}
...
seqs.forEach(seq => {
  ...
  // const isQuantized = mm.sequences.isQuantizedSequence(seq);
   seqText.innerHTML = '{ ' +
       seq.notes
           .map(n => {
             let s =   n.pitch + '[' +n.quantizedStartStep + ',';
             const end = n.quantizedEndStep ;
             if (end != null) {
               s +=  end;
             }
             s += ']';
