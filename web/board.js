const PIXELS_WIDTH = 64;
const PIXELS_HEIGHT = 38;
const MAX_PITCH = 83;
...
class Board {
  ...
  getNoteSequence() {
    const sequence = {notes:[], ticksPerQuarter: 220,totalTime: 1.5,
  ...
  drawNoteSequence(ns) {
    ...
    for(let n = 0; n < ns.notes.length; n++) {
      ...
      if(e > s+1 && e<=PIXELS_WIDTH)
      for(let i = s; i < e; i++)
      {
        this.data[r][i].on = 0;
        const uiButton = document.querySelector(`.pixel[data-row="${r}"][data-col="${i}"]`);
        this.voiceButton(uiButton, 0);
      }
      else{
        this.data[r][s].on = 0;
        const uiButton = document.querySelector(`.pixel[data-row="${r}"][data-col="${s}"]`);
        this.voiceButton(uiButton, 0);
      }
